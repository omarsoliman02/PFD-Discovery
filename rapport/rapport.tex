...
Le projet est organis\'{e} en modules Python ind\'{e}pendants et r\'{e}utilisables, r\'{e}partis sur 17 fichiers. L'architecture suit une s\'{e}paration nette entre le c\oe{}ur algorithmique, l'int\'{e}gration LLM et les scripts d'exp\'{e}rimentation.
...
\midrule
\multicolumn{2}{l}{\textit{C\oe{}ur algorithmique et integration LLM (\texttt{src/})}} \\
\midrule
\texttt{data\_loader.py} & Chargement CSV, introspection du sch\'{e}ma \\
\texttt{pattern\_extraction.py} & D\'{e}finition et application des 7 transformations \\
\texttt{pattern\_grouping.py} & Groupement des tuples par pattern \\
\texttt{candidate\_generation.py} & G\'{e}n\'{e}ration des paires candidats $(T_X, Y)$ \\
\texttt{validation.py} & Calcul du support et de la confidence \\
\texttt{generalization.py} & Fusion algorithmique + s\'{e}mantique (LLM) \\
\texttt{classical\_pipeline.py} & Orchestration du pipeline en 5 \'{e}tapes \\
\texttt{llm\_agent.py} & Mistral/Llama + 5 prompts (W1, W2, W3, gen.\ s\'{e}m.) \\
\texttt{agentic\_workflow.py} & Workflows agentiques 1, 2 et 3 \\
\texttt{evaluation.py} & M\'{e}triques et comparaison \\
\midrule
\multicolumn{2}{l}{\textit{Scripts d'exp\'{e}rimentation (\texttt{experiments/})}} \\
\midrule
\texttt{run\_classical.py} & Script classique \\
\texttt{run\_agentic.py} & Script W1 et W2 \\
\texttt{run\_workflow3.py} & Script W3 (Agent-in-the-Loop) \\
\texttt{compare\_results.py} & Comparaison classique vs W1/W2 \\
\texttt{sensitivity\_analysis.py} & Analyse de sensibilit\'{e} $K$ et $\theta$ \\
\texttt{error\_analysis.py} & Analyse d'erreur W2 \\
\texttt{run\_llm\_generalization.py} & G\'{e}n\'{e}ralisation s\'{e}mantique par LLM \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    \item Impl\'{e}mentation compl\`{e}te en Python avec 17 modules r\'{e}utilisables et bien d\'{e}coupl\'{e}s.
